...
type BlockType = "text" | "heading" | "ai-prompt" | "html" | "image" | "button";
...
const blockTypeConfig = {
  ...
  html: {
    ...
    label: 'HTML Block',
    description: 'Raw HTML for custom formatting',
    shortcut: 'M'
  },
  image: {
    icon: FeatherImage,
    label: 'Image',
    description: 'Embed an image in the email',
    shortcut: 'I'
  },
  button: {
    icon: FeatherLink,
...
function BlockComponent({ block, blocks, onUpdate, onDelete, onAddBlock, onShowSlashMenu, onShowVariableMenu }: BlockComponentProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    ...
    } else if (e.key === 'Enter' && !e.shiftKey && validBlockType !== 'ai-prompt' && validBlockType !== 'html') {
      e.preventDefault();
  ...
  const getPlaceholder = () => {
    switch (validBlockType) {
      ...
      case 'html': return "Enter HTML markup for custom formatting...";
      case 'image': return "Enter image URL...";
      case 'button': return "Enter button text...";
  ...
  const renderInput = () => {
      ...
      return (
        <div className="flex flex-col gap-2 w-full">
          ...
          <input
            ...
            className={`${baseClasses} text-caption font-caption text-subtext-color`}
    ...
    return (
      <textarea
        ...
        onBlur={() => setIsFocused(false)}
        onKeyDown={handleKeyDown}
        rows={validBlockType === 'ai-prompt' || validBlockType === 'html' ? 3 : 1}
        className={`${baseClasses} text-body font-body text-default-font ${
          validBlockType === 'ai-prompt' ? 'text-brand-700' : ''
        } ${validBlockType === 'html' ? 'font-mono text-neutral-700' : ''}`}
        ...
        role="textbox"
        aria-multiline={!['heading', 'button', 'image'].includes(validBlockType)}
      />
    );
  };
  ...
  const getBlockContainerClasses = () => {
    ...
    if (validBlockType === 'html') {
      return `${baseClasses} ${isFocused ? 'bg-neutral-25 border border-neutral-300' : 'bg-neutral-25/50 border border-transparent hover:border-neutral-200'}`;
    }
            ...
            <div className="flex w-full flex-col gap-6 rounded-lg border border-solid border-neutral-border bg-default-background p-6">
              ...
              <div className="border-b border-neutral-100 pb-6">
                <div className="flex items-center gap-2 mb-3">
                  <FeatherMail className="w-4 h-4 text-brand-600" />
                ...
                <input
                  ...
                  role="textbox"
                />
                <span className="text-caption font-caption text-subtext-color mt-1 block">
                  Use @ to insert variables in your subject line
                </span>
              </div>
                  ...
                  {blocks.map((block) => (
                    <div key={block.id}>
                      ...
                      {block.type === 'html' && block.content && (
                        <div className="rounded border border-solid border-neutral-200 bg-neutral-50 p-3">
                          <div className="text-caption-bold font-caption-bold text-neutral-700 mb-1">HTML Block:</div>
                      ...
                      {block.type === 'image' && block.content && (
                        <div className="rounded border border-solid border-neutral-200 bg-neutral-50 p-3 text-center">
                          <FeatherImage className="w-8 h-8 text-neutral-400 mx-auto mb-2" />
                          <div className="text-caption font-caption text-neutral-600">{block.content}</div>
                          {block.alt && (
                            <div className="text-caption font-caption text-neutral-500 mt-1">Alt: {block.alt}</div>
                          )}
                        </div>
                      )}
                      {block.type === 'button' && block.content && (
                        <div className="text-center">
                          ...
                          {block.url && (
                            <div className="text-caption font-caption text-neutral-500 mt-1">{block.url}</div>
                          )}
                        </div>
                      )}
                    </div>
